...
float dice(vec3 p, out float dots) {
    float cube = distRoundBox(p, vec3(cubeSize),cubeRadius);
    dots = min(dots,addDots(p));
    return min(cube,dots);
}

float chain(vec3 p){
    vec3 pt = rotate(p, vec3(90,0,0));
    float t2 = distTorus(repeat(pt, vec3(0,0,3)),vec2(1,0.2));
    vec3 pt2 = rotate(p, vec3(90,45,0));
...
float dist(vec3 p){
    ...
    p0 = rotate(p0, vec3(0,rot * p.y/divide,0));
    p0 = rotate(p0, vec3(45, 0, 45));
    float dices = dice(p0, dots0); /*
    dices = min(dices, dice(p0 + vec3(3.75,0,0), dots0));
    //p0 = rotate(p0, vec3(180-rot * p.y / 500));
    dices = min(dices, dice(p0 + vec3(3.75,-3.75,0), dots0));
    dices = min(dices, dice(p0 + vec3(0,-3.75,0), dots0));*/
    //float torus = distTorus(pt, vec2(3,1));

    vec3 pc = rotate(p, vec3(0,0,90));
    float ch = chain(pc+vec3(-4,0,2));
    float res = min(ch, min(plane,dices));
    vec3 color = mix(vec3(0.2,0.2,0.8), vec3(0,0.35,0.3),p.y);
    vec3 buttonColor = mix(vec3(0.7,0.7,0.4), vec3(0,0.6,0.6),p0.z);
    objColor = (res == dots0) ?  vec4(buttonColor,1) : vec4(color,1);   
    objColor = (res == plane || res == ch) ?  vec4(0.3) : objColor;  
    //objColor = (res == t2) ?  vec4(0.3) : objColor;
    return res;
}
