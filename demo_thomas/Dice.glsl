...
float addDots(in vec3 p) {	
	...
	float dots = min(one,min(two,min(three, min(four, min(five, seven)))));	
...
float dist(vec3 p){
	...
	float dices = dice(repeat(p0, vec3(5)), dots0);
	float res = min(plane,dices);
	vec3 color = mix(vec3(0.2,0.2,0.8), vec3(0,0.35,0.3),p0.y);
