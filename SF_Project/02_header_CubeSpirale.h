...
float dist2(vec3 p){
	p = rotate2(p, vec3(spiralTime*20+90, spiralTime*20-90, spiralTime*20));
...
vec4 getSpiralColor(vec2 resolution, float time){
	spiralTime = time;

	float tanFov = tan(90.0 / 2.0 * 3.14159 / 180.0) / resolution.x;
    vec2 p = tanFov * (gl_FragCoord.xy * 2.0 - resolution.xy);

	camSpiral.pos = vec3(0,0,-10);
	camSpiral.dir = normalize(vec3(p.x, p.y, 1));
	int steps = -1;
	vec4 res = raymarch2(camSpiral.pos, camSpiral.dir, steps);
