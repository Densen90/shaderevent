...
vec3 lightDir6 = normalize(vec3(0.5,0.5,-1));
float shaderTime6;
...
float ambientOcclusion6(vec3 p, vec3 n)
{
    float res = 0.0;
    float fac = 1.0;
...
vec4 getShader6Color(vec2 resolution, float time)
{
    shaderTime6 = time;
    ...
    cam6.pos = vec3(0 , 0 , 0);
    cam6.dir = normalize(vec3(p.x, p.y, 1.0));  

    // marching and shading
	int steps = -1;
    vec4 res = raymarch6(cam6.pos, cam6.dir, steps);    
	
	return vec4(0.5, 0.9, 0.5, 1);
}
