...
{
	//p = abs(sin(p));#
	float box2 = sdTorus88(Repeat(p + vec3(1), vec3(.0, 5, 1.0)), vec2(5.0, 15.0));
	p = rotate(p, vec3(iGlobalTime , 0.0, iGlobalTime* 5));
	color = ColorScene(p/2);
	float box = sdTorus88(Repeat(p, vec3(8, 5, 2)), vec2(.0, 2.0));
	float temp = fOpDifferenceStairs(box, box2, 8, -1);
	return fOpDifferenceStairs(box, box2, 8, -1);
}
